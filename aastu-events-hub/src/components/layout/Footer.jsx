        ...
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 32, marginBottom: 40 }}>
            ...
            <div>
              ...
              {[
                { label: 'Help Center', action: () => setShowHelp(true) },
                { label: 'Privacy Policy', to: '#' },
                { label: 'Terms of Service', to: '#' },
                { label: 'Contact Support', to: 'mailto:azyirga@gmail.com' },
              ].map(item => (
                item.action ? (
                  <button key={item.label} onClick={item.action} style={{
                    display: 'block', fontSize: 13, color: '#64748B',
                    background: 'none', border: 'none', cursor: 'pointer',
                    marginBottom: 8, padding: 0, textAlign: 'left', transition: 'color 0.15s',
                  }}
                    onMouseEnter={e => e.currentTarget.style.color = '#fff'}
                    onMouseLeave={e => e.currentTarget.style.color = '#64748B'}
                  >{item.label}</button>

...
export default function StatusBadge({ event, style = {} }) {
  ...
  if (status === 'soon') {
    return (
      <span className="badge badge-amber" style={{ animation: 'pulse-amber 1.5s ease-in-out infinite', ...style }}>
        ⚡ STARTING SOON
      </span>
    );
  }
  if (status === 'ended') {
    return <span className="badge badge-gray" style={style}>ENDED</span>;
  }
